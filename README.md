...
3. Create an application in Slack using the modern "granular" permissions. Take note of the verification token; this is `VERIFICATION_TOKEN`.
...
6. Provide configuration via environment variables somehow. LOUDBOT will read a .env file if one is available in its working directory.
...
10. Subscribe to these bot events: `message.channels` and `reaction_added`.
11. SHOUT AT LOUDBOT. IT SHOULD SHOUT BACK.
